...
const HEADER_HEIGHT = 66; // Header height + safe area
const BOTTOM_SHEET_MIN_HEIGHT = 200;
// Max height should be conservative - around 45% of screen or content-based
// This prevents the bottom sheet from going too far up and creating blank space
// Each timeline item is ~100px, so 5 items + header = ~600px max
const ESTIMATED_CONTENT_HEIGHT = 600; // Drag handle (24) + title (50) + 5 items (~100 each) + padding
const BOTTOM_SHEET_MAX_HEIGHT = Math.min(
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        ...
        const newValue = Math.max(maxTranslate, Math.min(0, gestureState.dy));
        translateY.setValue(newValue);
        currentTranslate.current = newValue;
      },
      onPanResponderRelease: (_, gestureState) => {
        translateY.flattenOffset();
        const currentValue = currentTranslate.current + currentOffset.current;
        const maxTranslate = -(BOTTOM_SHEET_MAX_HEIGHT - BOTTOM_SHEET_MIN_HEIGHT);
        ...
        if (gestureState.dy < -50 || (currentValue < midPoint && gestureState.vy < -0.5)) {
          ...
          // Snap to nearest
          targetValue = currentValue < midPoint ? maxTranslate : 0;
        }

        currentOffset.current = targetValue;
        currentTranslate.current = 0;
